...
- Docker Desktop已安裝
- 至少8GB RAM、4個CPU核心分配給Docker
- 執行命令的工具（終端或PowerShell）
- 基本的JSON和API知識
...
services:
  opensearch-node1:
    ...
    environment:
      ...
      - plugins.ml_commons.only_run_on_ml_node=false  # 允許ML模型在單節點上運行
    ulimits:
      memlock:
        soft: -1
        hard: -1
  ...
  opensearch-dashboards:
    ...
    environment:
      ...
      - ml_commons_dashboards.enabled=true  # 啟用ML模型
    networks:
      - opensearch-net
    depends_on:
      - opensearch-node1
...
```bash
# 檢查API連通性
curl -XGET https://localhost:9200 -u 'admin:YourStrongPassword' --insecure

# 確認ML Commons插件已安裝
curl -XGET https://localhost:9200/_cat/plugins -u 'admin:YourStrongPassword' --insecure
```
...
```bash
curl -XPUT https://localhost:9200/_cluster/settings -u 'admin:YourStrongPassword' -H 'Content-Type: application/json' -d '{"persistent":{"plugins.ml_commons.allow_registering_model_via_url": true}}' --insecure
```

### 2. 創建模型組

```bash
curl -XPOST https://localhost:9200/_plugins/_ml/model_groups/_register -u 'admin:YourStrongPassword' -H 'Content-Type: application/json' -d '{"name": "text_embedding_group", "description": "Models for text embedding"}' --insecure
```

記錄返回的`model_group_id`。

### 3. 註冊預訓練模型

```bash
curl -XPOST https://localhost:9200/_plugins/_ml/models/_register -u 'admin:YourStrongPassword' -H 'Content-Type: application/json' -d '{
...
```bash
curl -XGET https://localhost:9200/_plugins/_ml/tasks/YOUR_TASK_ID -u 'admin:YourStrongPassword' --insecure
```

等待狀態變為`COMPLETED`，然後記錄返回的`model_id`。

### 5. 部署模型

```bash
curl -XPOST https://localhost:9200/_plugins/_ml/models/YOUR_MODEL_ID/_deploy -u 'admin:YourStrongPassword' --insecure
```
...
```bash
curl -XPUT https://localhost:9200/semantic-search -u 'admin:YourStrongPassword' -H 'Content-Type: application/json' -d '{
  "mappings": {
    "properties": {
      "content": { 
        "type": "text" 
